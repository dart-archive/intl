...
/// It supports syntax to represent plurals and select options.<br/>
/// Think of it as "a locale aware printf."
///
...
/// produce `"I see Mark and one other person in Athens."` as output.<br/>
...
/// numbers" which have only a loose connection to the numerical value.<br/>
...
///
/// This class documentation is not the right place too explore the intricancies
/// of plurals in foreign languages.<br/>
...
// are clasified as `blocks` and will be parsed again (recursively)
//
...
class MessageFormat {
  ...
  /// The [pattern] parameter is the one we parse and apply positional
  /// parameters to.
  ...
  /// Returns a formatted message, treating '#' with special meaning
  /// representing the number (plural_variable - offset).
  ///
  ...
  String _format(bool ignorePound, [Map<String, Object> namedParameters]) {
    ...
    _literals = ListQueue<String>();
    _literals.addAll(_initialLiterals);

    var messageParts = ListQueue<String>();
  ...
  /// The [parsedPattern] parameter holds parsed tree.
  ...
  void _formatBlock(
      ListQueue<_BlockTypeAndVal> parsedPattern,
      Map<String, Object> namedParameters,
      bool ignorePound,
      ListQueue<String> result) {
    for (var currentPattern in parsedPattern) {
  ...
  /// [parsedPattern] is an object containing placeholder info.
  /// The [namedParameters] that are used as actual data.
  /// Each formatting stage appends its product to the [result].
  void _formatSimplePlaceholder(String parsedPattern,
      Map<String, Object> namedParameters, ListQueue<String> result) {
    var value = namedParameters[parsedPattern];
    if (!_isDef(value)) {
      result.add('Undefined parameter - $parsedPattern');
      return;
    }
    ...
    String strValue;
    if (value is int) {
      strValue = _numberFormatter.format(value);
  ...
  /// [parsedPattern] is an object containing select block info.
  /// [namedParameters] are parameters that either influence the formatting
  /// or are used as actual data.
  /// If [ignorePound] is true, treat '#' in plural messages as a
  /// literary character, else treat it as an ICU syntax character, resolving
  ...
  void _formatSelectBlock(
      Map<String, Object> parsedPattern,
      Map<String, Object> namedParameters,
      bool ignorePound,
      ListQueue<String> result) {
    var argumentIndex = parsedPattern['argumentIndex'];
    if (!_isDef(namedParameters[argumentIndex])) {
      result.add('Undefined parameter - $argumentIndex');
      return;
    }

    var option = parsedPattern[namedParameters[argumentIndex]];
    if (!_isDef(option)) {
      option = parsedPattern[_other];
  ...
  /// Formats plural or selectordinal block. Only one option is selected and
  /// all # are replaced.
  /// [parsedPattern] is an object containing plural block info.
  /// [namedParameters] are parameters that either influence the formatting
  /// or are used as actual data.
  ...
  void _formatPluralOrdinalBlock(
    ...
    if (numPluralValue == null) {
      result.add('Invalid parameter - $argumentIndex');
      return;
    }
    ...
    var option = parsedPattern[namedParameters[argumentIndex]];
    if (!_isDef(option)) {
      option = parsedPattern[namedParameters[argumentIndex].toString()];
    }
    if (!_isDef(option)) {
    ...
    var pluralResult = ListQueue<String>();
    ...
    if (ignorePound) {
      ...
      var localeAwareDiff = _numberFormatter.format(diff);
  ...
  void _init() {
    if (_pattern != null) {
      _initialLiterals = ListQueue<String>();
  ...
  ListQueue<_ElementTypeAndVal> _extractParts(String pattern) {
    var prevPos = 0;
    var braceStack = ListQueue<String>();
    var results = ListQueue<_ElementTypeAndVal>();
  ...
  ListQueue<_BlockTypeAndVal> _parseBlock(String pattern) {
    var result = ListQueue<_BlockTypeAndVal>();
  ...
  Map<String, Object> _parseSelectBlock(String pattern) {
    ...
    Map<String, Object> result = {'argumentIndex': argumentIndex};
    ...
    while (pos < parts.length) {
      ...
      ListQueue<_BlockTypeAndVal> value;
  ...
  Map<String, Object> _parseOrdinalBlock(String pattern) {
    ...
    var result = {'argumentIndex': argumentIndex, 'argumentOffset': 0};
  ...
  String _buildPlaceholder(ListQueue<String> literals) {
